...
class sfYamlParser
{
   ...
   *
   * @throws TMParameterException If the YAML is not valid
   */
  public function parse($value)
  {
    ...
    while ($this->moveToNextLine())
    {
      ...
      {
        throw new TMParameterException(sprintf('A YAML file cannot contain tabs as indentation at line %d (%s).', $this->getRealCurrentLineNb() + 1, $this->currentLine));
      }
      ...
      else if (preg_match('#^(?P<key>'.sfYamlInline::REGEX_QUOTED_STRING.'|[^ ].*?) *\:(\s+(?P<value>.+?))?\s*$#', $this->currentLine, $values))
      {
        ...
        if ('<<' === $key)
        {
          if (isset($values['value']) && '*' === substr($values['value'], 0, 1))
          {
            ...
            {
              throw new TMParameterException(sprintf('Reference "%s" does not exist at line %s (%s).', $isInPlace, $this->getRealCurrentLineNb() + 1, $this->currentLine));
            }
          }
          else
          {
            ...
            if (!is_array($parsed))
            {
              throw new TMParameterException(sprintf("YAML merge keys used with a scalar value instead of an array at line %s (%s)", $this->getRealCurrentLineNb() + 1, $this->currentLine));
            }
            else if (isset($parsed[0]))
            {
              ...
              foreach (array_reverse($parsed) as $parsedItem)
              {
                ...
                {
                  throw new TMParameterException(sprintf("Merge items must be arrays at line %s (%s).", $this->getRealCurrentLineNb() + 1, $parsedItem));
                }
      ...
      else
      {
        ...
        throw new TMParameterException(sprintf('%s %d (%s).', $error, $this->getRealCurrentLineNb() + 1, $this->currentLine));
      }
  ...
  protected function getNextEmbedBlock()
  {
    ...
    {
      throw new TMParameterException(sprintf('Indentation problem at line %d (%s)', $this->getRealCurrentLineNb() + 1, $this->currentLine));
    }
  ...
  protected function parseValue($value)
  {
    if ('*' === substr($value, 0, 1))
    {
      ...
      {
        throw new TMParameterException(sprintf('Reference "%s" does not exist (%s).', $value, $this->currentLine));
      }
